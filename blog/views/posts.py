...
def model_form_data_create(request, form_arg: str):
    if request.method == "POST":
        if form_arg == "post-create-form":
            ...
            image_create_form = ImageForm(
                request.POST, request.FILES, initial={"file": image_file}
            )
            ...
            if category_create_form.is_valid():
                ...
                messages.error(
                    request, "Blog creation failed due to invalid category form"
                )
                return redirect("blog:post-forms-page")
...
@login_required(login_url=settings.LOGIN_REDIRECT_URL)
def post_update(
    request, title: str = None, pk: int = 0, form_arg: str = None, *args, **kwargs
):
    ...
    if request.method == "POST":
        if form_arg == "post-update-form":
            ...
            if blog_post_form.is_valid():
                ...
                messages.error(
                    request, "blog post creation failed due to invalid blog post form"
                )
                return redirect("blog:post-forms-page")
            ...
            if category_update_form.is_valid():
                ...
                messages.error(
                    request, "category update failed due to invalid category form"
                )
                return redirect("blog:post-forms-page")
        else:
            pass
